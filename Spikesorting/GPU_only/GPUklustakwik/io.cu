...
void KK::LoadData(char *FileBase, int ElecNo, char *UseFeatures)
{
	...
	if (Debug) Output("Number of feature read in : %d \n ", nFeatures);
	...
	for (i = 0; i < nDims; i++)
	{
		...
		for (p = 0; p < nPoints; p++)
			Data[p*nDims + i] = (Data[p*nDims + i] - min) / (max - min);
	}

	//cudaMemcpy(d_Data, &Data[0], nPoints*nDims*sizeof(float), cudaMemcpyHostToDevice);
	//cudaMemcpy(d_FloatMasks, &FloatMasks[0], nPoints*nDims* sizeof(float), cudaMemcpyHostToDevice);
	//cudaMemcpy(d_Masks, &Masks[0], nPoints*nDims * sizeof(int), cudaMemcpyHostToDevice);
