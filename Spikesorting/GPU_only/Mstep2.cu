...
__global__ void calCov(int nClustersAlive, int nPoints, int nDims, int priorPoint,int *d_Class, int *d_AliveIndex, int *d_mark,
	int **d_PointsInClass,
	int **d_ClusterFeatures, int *d_nClassMembers) {

	int tidx = blockDim.x * blockIdx.x + threadIdx.x;


	//__shared__ float d_mark[128];

	if (tidx < nClustersAlive) {
		//int offsets = bidx * offset;
		int c = d_AliveIndex[tidx];

		int *d_ClusterFeature = *(d_ClusterFeatures[c]);
		int *d_PointsInThisClass = *(d_PointsInClass[c]);

		thrust::inclusive_scan(thrust::device, d_mark, d_mark + nDims, d_mark);
		int nunmasked = (int)d_mark[nDims - 1];
		int nmasked = nDims - nunmasked;
		int npoints = d_nClassMembers[c];
		float* d_cov = new float[nunmasked * nunmasked];
		float* d_dig = new float[nmasked];

		if (nunmasked > 0 && npoints > 0) {
			float* d_X = new float[npoints * nunmasked];
			matCopy << <grid, block >> > (npoints, nunmasked, nDims,
				(*d_PointsInThisClass)[c],
				d_CurrentUnmasked,
				d_AllVector2Mean,
				d_X);
			...
			cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, nunmasked, nunmasked, npoints, &alpha,
				d_X, nunmasked, d_X, nunmasked, &beta,
				d_cov, nunmasked);
		...
		c_CorrectionTerm << < BLOCKDIM, BLOCKDIM >> > (nDims, nunmasked, nmasked, npoints,
			d_CurrentUnmasked,
			d_CurrentMasked,
			d_cov,
			d_dig,
		...
		addNoiseVar << <BLOCKDIM, BLOCKDIM >> > (nunmasked, nmasked, priorPoint,
			d_NoiseVariance,
			d_CurrentUnmasked,
			d_CurrentMasked,
			d_cov,
		...
		float alpha = 1. / (d_nClassMembers[c] + priorPoint - 1);
		cublasSscal(handle1, nunmasked*nunmasked, &alpha, d_cov, 1);
		cublasSscal(handle1, nmasked, &alpha, d_dig, 1);
...
void KK::MStep()
{
	...
	thrust::device_vector<int> d_mark(MaxPossibleClusters,0);
	c_Reindex<<<,BLOCKDIM>>>(nClustersAlive, MaxPossibleClusters,
		thrust::raw_pointer_cast(&d_AliveIndex[0]),
		thrust::raw_pointer_cast(&d_ClassAlive[0]),
